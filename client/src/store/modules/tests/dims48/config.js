...
export default {
  ...
  getters: {
    ...
    getLeftBtnKey: (state, getters, rootState) => {
      return keycodes(parseInt(state.leftBtnKeyCode)).replace(
        "left",
        "Linker pijl"
      );
    },
    ...
    getRightBtnKey: (state, getters, rootState) => {
      return keycodes(parseInt(state.rightBtnKeyCode)).replace(
        "right",
        "Rechter pijl"
      );
    }
  ...
  actions: {
    ...
    updateConfigValues: ({ commit, state, rootState }, newConfig) => {
      commit(
        "updateInterferenceDurationLocal",
        newConfig["interferenceDuration"]
      );
      commit(
        "updatePhase1SecondsPerImageLocal",
        newConfig["phase1SecondsPerImage"]
      );
